...
abstract class REST_Controller extends \CI_Controller {
    ...
    protected function _perform_ldap_auth($username = '', $password = NULL)
    {
        ...
        $badchars = array("\n", "\r", "\t");
$safedata = str_replace($badchars, "", $username);

        // Search for user
        if (($res_id = ldap_search($ldapconn, $ldap['basedn'], "uid=$safedata")) === FALSE)
        {
            log_message('error', 'LDAP Auth: User '.$safedata.' not found in search');
            return FALSE;
        }

        if (ldap_count_entries($ldapconn, $res_id) !== 1)
        {
            log_message('error', 'LDAP Auth: Failure, username '.$safedata.'found more than once');
            return FALSE;
        }
...
$safedata = str_replace($badchars, "", $username);
        ...
        if ($auth_source === 'ldap')
        {
            log_message('debug', "Performing LDAP authentication for $safedata");
        ...
        {
            log_message('debug', "Performing Library authentication for $safedata");
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match("/(?:,\s*|^)\Q%s\E(?=,\s*|$)/m", $this->config->item('rest_ip_blacklist')))
        {
            // Display an error response
            $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
